...
def finvasia_get_pnl():
    ...
    if response.status_code == 200:
        ...
        pnl = 0
        logger.info(f"Daily P&L Json : {response_json} ")
        for i in response_json:
            #mtm += float(i['urmtom'])
            pnl += float(i['rpnl'])
        return pnl   
        logger.info(f"{pnl} is your Daily P&L")

        
    else:
        error_message = response.json()["emsg"]
        logger.warning(f"fetch positions warning: {error_message}")
...
def get_broker_pnl():
    try:
        if Config.API.provider == ApiProvider.SHOONYA:
            try:
                return finvasia_get_pnl()
            except:
                logger.warning(f"Error in getting pnl.")
                logger.debug(f"Error in getting pnl", exc_info=True)
    except Exception as ex:
        logger.debug(f"Error in getting pnl", exc_info=True)
        raise ex
