...
from instruments import get_exchnage_token, get_fyers_tradingsymbol, get_groww_tradingsymbol, get_instrument, get_zerodha_tradingsymbol, load_fyers_instruments
...
def fyers_place_order(order: Order, api: Api):
    ...
    try:
        ...
        if response.status_code == 200 and response.json()["s"] == "ok":
            ...
            logger.info(f"Order places with broker id, {broker_order_id}")
        else:
